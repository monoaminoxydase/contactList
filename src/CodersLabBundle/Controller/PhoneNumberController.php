...
class PhoneNumberController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $phoneNumbers = $em->getRepository('CodersLabBundle:PhoneNumber')->findAll();

        return $this->render('CodersLabBundle:phonenumber:index.html.twig', array(
     ...
     *
     * @Route("/new", name="phonenumber_new")
     * @Method({"GET", "POST"})
     */
    public function newAction(Request $request)
    {
        $phoneNumber = new Phonenumber();
        $form = $this->createForm('CodersLabBundle\Form\PhoneNumberType', $phoneNumber);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->persist($phoneNumber);
            $em->flush($phoneNumber);

            return $this->redirectToRoute('phonenumber_show', array('id' => $phoneNumber->getId()));
        }

        return $this->render('CodersLabBundle:phonenumber:new.html.twig', array(
    ...
    {
        $deleteForm = $this->createDeleteForm($phoneNumber);
        $editForm = $this->createForm('CodersLabBundle\Form\PhoneNumberType', $phoneNumber);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('phonenumber_edit', array('id' => $phoneNumber->getId()));
        }

        return $this->render('CodersLabBundle:phonenumber:edit.html.twig', array(
            'phoneNumber' => $phoneNumber,
